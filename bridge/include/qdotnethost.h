...
class QDotNetHost {
    public:
        ...
        QMap<QString, QString> runtimeProperties() const {
            ...
            const char_t** keys = new const char_t* [count] {
                nullptr
            };
            const char_t** values = new const char_t* [count] {
                nullptr
            };
...
\bMicrosoft\.NETCore\.App[^0-9]*(?<version>[0-9\.\-rc]+)[^\[]*\[(?<path>[^\]]+)\]
)[regex]")
                                                               .remove('\r')
                                                               .remove('\n');
